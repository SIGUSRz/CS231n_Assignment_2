...
def affine_backward(dout, cache):
    ...
    #############################################################################
    N = x.shape[0]
    scores = x.ravel(order='C')
    D = scores.shape[0] / N
    scores = scores.reshape(N, D)
    dw = scores.T.dot(dout)
...
def batchnorm_backward(dout, cache):
    ...
    #############################################################################
    norm_out, x, gamma, mean, var, eps = cache
    N, D = dout.shape
    dgamma = np.sum(np.multiply(dout, norm_out), axis=0)
    dbeta = np.sum(dout, axis=0)
    dnorm_x = np.multiply(dout, gamma)
    dvariance = np.multiply(x - mean, (-0.5) * (var + eps) ** (-1.5))
    dvariance = np.sum(np.multiply(dnorm_x, dvariance), axis=0)
    dmean = np.sum(np.multiply(dnorm_x, -1 / np.sqrt(var + eps)), axis=0)
    dx = np.multiply(dnorm_x, 1 / np.sqrt(var + eps)) + \
         np.multiply(dvariance, (x - mean) * 2 / N) + \
         dmean / N
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    out_batch, cache = batchnorm_forward(x_batch,gamma,beta,bn_param)
    out = out_batch.reshape(N,H,W,C).transpose(0,3,1,2)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx_batch, dgamma,dbeta = batchnorm_backward_alt(dout_batch,cache)
